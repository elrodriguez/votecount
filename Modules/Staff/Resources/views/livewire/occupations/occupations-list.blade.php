<div>
    <div class="card mb-g rounded-top">
        <div class="card-header">
            <div class="input-group bg-white shadow-inset-2">
                ...
                <div class="input-group-append">
                    <button wire:click="occupationsSearch" class="btn btn-default waves-effect waves-themed" type="button">@lang('staff::labels.btn_search')</button>
                    @can('personal_ocupaciones_nuevo')
                    <a href="{{ route('staff_occupation_create') }}" class="btn btn-success waves-effect waves-themed" type="button">@lang('staff::labels.btn_new')</a>
                    @endcan
                </div>
            </div>
        </div>
        <div class="card-body p-0">
            <table class="table m-0">
                ...
                <tr>
                    ...
                    <th>@lang('staff::labels.lbl_description')</th>
                ...
                @foreach($occupations as $key => $occupation)
                    <tr>
                        ...
                        <td class="text-center tdw-50 align-middle">
                            <div class="btn-group">
                                ...
                                <div class="dropdown-menu" style="position: absolute; will-change: top, left; top: 35px; left: 0px;" x-placement="bottom-start">
                                    @can('personal_ocupaciones_editar')
                                    <a href="{{ route('staff_occupation_edit',$occupation->id) }}" class="dropdown-item">
                                        <i class="fal fa-pencil-alt mr-1"></i>@lang('staff::labels.btn_edit')
                                    </a>
                                    @endcan
                                    @can('personal_ocupaciones_eliminar')
                                    <div class="dropdown-divider"></div>
                                    <button onclick="confirmDelete({{ $occupation->id }})" type="button" class="dropdown-item text-danger">
                        ...
                        <td class="align-middle">{{ $occupation->name }}</td>
        ...
        <div class="card-footer  pb-0 d-flex flex-row align-items-center">
            <div class="ml-auto">{{$occupations->links()}}</div>
        </div>
    </div>
